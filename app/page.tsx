...
export default function HomePage() {
	...
	const novenasAtivas = novenasComProgresso.filter(
		(n) => !n.progress.isFinished,
	);
	const novenasConcluidas = novenasComProgresso.filter(
		(n) => n.progress.isFinished,
	);
			...
			{showCreateForm && (
				<div className="mb-8 p-4 sm:p-6 bg-gray-50 rounded-lg border">
					...
					<div className="flex flex-col gap-4 w-full">
						<div>
							<label
								className="text-sm block font-medium text-indigo-900 mb-1"
								htmlFor="novena-select"
							>
								Selecione uma novena
							</label>
							<select
								className="border p-3 w-full rounded-lg text-base"
								value={selectedNovena}
								onChange={(e) => setSelectedNovena(e.target.value)}
							>
								...
								<option value="beato-padre-eustaquio">
									Novena ao Beato Padre Eustáquio
								</option>
							</select>
						</div>
						...
						<div>
							<label
								className="text-sm font-medium block text-indigo-900 mb-1"
								htmlFor="data-inicio"
							>
								Data de início
							</label>
				...
				(novenasAtivas.length > 0 ? (
					<div className="space-y-4">
						...
						{novenasAtivas.map((novena) => {
							...
							const needsToPrayToday =
								!novena.completedDays.includes(novena.progress.currentDay) &&
								novena.progress.currentDay <= 9;
							return (
								<div
									key={novena.id}
								...
								>
										...
										<div className="absolute -top-3 right-3 flex items-center gap-1 animate-bounce z-10">
											<span className="inline-flex items-center px-2 py-0.5 rounded-full bg-green-500 text-white font-semibold text-xs shadow-lg border border-green-600">
												<svg
													...
													viewBox="0 0 20 20"
												>
													<path d="M10 2a1 1 0 01.894.553l7 14A1 1 0 0117 18H3a1 1 0 01-.894-1.447l7-14A1 1 0 0110 2zm0 3.618L4.618 16h10.764L10 5.618zM9 8h2v4H9V8zm0 6h2v2H9v-2z" />
												</svg>
												Reze o dia de hoje
											</span>
										</div>
									...
									<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 gap-3">
										<div className="flex-1">
											...
											<div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-sm text-gray-600">
												<span>
													Início:{' '}
													{format(parseISO(novena.startDate), 'dd/MM/yyyy', {
														locale: ptBR,
													})}
												</span>
									...
									<div className="w-full bg-gray-200 rounded-full h-2">
										<div
											...
											style={{
												width: `${(novena.progress.completed / 9) * 100}%`,
											}}
										></div>
									</div>
								</div>
					...
					!showCreateForm && (
						<div className="text-center py-12">
							...
							<h3 className="text-lg font-medium text-gray-900 mb-2">
								Nenhuma novena ativa
							</h3>
							<p className="text-gray-500 mb-4">
								Comece uma nova jornada espiritual clicando no botão acima.
							</p>
						</div>
					)
				))}
			...
			{tab === 'concluidas' &&
				(novenasConcluidas.length > 0 ? (
					<div className="space-y-4">
						...
						{novenasConcluidas
							.slice(0, concluidasPage * CONCLUIDAS_PER_PAGE)
							.map((novena) => (
								<div
									key={novena.id}
									className="p-4 bg-green-50 border border-green-200 rounded-lg shadow-sm opacity-80 hover:opacity-100 transition-opacity"
								>
									<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 gap-3">
										<div className="flex-1">
											<h3 className="font-semibold text-lg mb-2 flex items-center gap-2">
												{novena.novenaData.name}
												<span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-200 text-green-800 ml-2">
													Concluída
												</span>
											</h3>
											<div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-sm text-gray-600">
						...
						{novenasConcluidas.length >
							concluidasPage * CONCLUIDAS_PER_PAGE && (
					...
					<div className="text-center py-12">
						...
						<h3 className="text-lg font-medium text-gray-900 mb-2">
							Nenhuma novena concluída
						</h3>
						<p className="text-gray-500 mb-4">
							Ao concluir uma novena, ela aparecerá aqui.
						</p>
					</div>
				))}
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
					<div className="bg-white rounded-lg shadow-lg p-6 max-w-xs w-full">
						...
						<p className="text-gray-700 mb-4 text-sm">
							Tem certeza que deseja excluir esta novena? Esta ação não pode ser
							desfeita.
						</p>
