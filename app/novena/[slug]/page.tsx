		...
		<section className="mt-6">
			...
			<div className="text-gray-600 mb-4 text-sm sm:text-base">
				<div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
					<span>
						Início:{' '}
						{format(parseISO(novenaInstance.startDate), 'dd/MM/yyyy', {
							locale: ptBR,
						})}
					</span>
					...
					{!isCurrentDay && canViewDay && (
						<>
							...
							<span className="text-blue-600">
								Visualizando: Dia {displayDayNumber}
							</span>
						</>
					)}
				</div>
			...
			{!canViewDay ? (
				<div className="mb-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
					<p className="text-yellow-800">
						Este dia da novena ainda não está disponível. Aguarde até{' '}
						{novenaInstance.startDate} para começar.
					</p>
				</div>
			) : (
				<>
					<div className="prayer-card mb-4 p-4 sm:p-6 bg-white rounded-lg border shadow-sm">
						...
						<div className="space-y-4 sm:space-y-6">
							...
							{current.prayerOfDay && (
								<div>
									<h3 className="text-base sm:text-lg mb-3">Oração do dia</h3>
									<p className="text-gray-800 whitespace-pre-line leading-relaxed text-sm sm:text-base">
										{current.prayerOfDay}
									</p>
								</div>
